...
// 塩尻市マスタ「返礼品マスタ」の、変更種別ごとの反映先の列
const MASTER_COLUMNS = {
  stock: "T列 在庫数",
  price: "Y列 精算額",
  shippingPeriod: "AE列 発送開始日 / AF列 発送終了日",
  acceptPeriod: "（対応列なし。C列備考へ記載）",
  desiredDate: "（対応列なし。C列備考へ記載）",
};
...
export default function handler(req, res) {
  ...
  if (type === "stock") {
    if (payload.action === "pause") {
      ...
      summary = `在庫数を ${delta > 0 ? "+" : ""}${delta} 調整（${before} → ${after}）`;
    }
  } else if (type === "price") {
    ...
    summary = `商品代を ${before} → ${after} に変更`;
  } else if (type === "desiredDate") {
    if (!isDate(payload.desiredDate)) return res.status(400).json({ error: "変更希望日が不正です" });
    before = "-";
    after = payload.desiredDate;
    summary = `変更希望日: ${payload.desiredDate}（お時間がかかる場合がございます）`;
  ...
  const record = {
    ...
    larkMessage:
      ...
      `申請日時: ${requestedAt}`,
    masterUpdate: {
      column: MASTER_COLUMNS[type],
      remark: `[${requestedAt}] ${session.name} がポータルから変更 / ${LABELS[type]}: ${before} → ${after}`,
    },
  };
